...
class VocabExtractor {
  ...
  static List<VocabCandidate> _extractKanjiTerms(String text) {
    ...
    final kanjiRegex = RegExp(r'[\u4E00-\u9FFF\u3400-\u4DBF\u20000-\u2A6DF]{2,8}');
  ...
  static bool _shouldExcludeTerm(String term) {
    ...
    // 記号のみ
    if (RegExp(r'^[^\w\u4E00-\u9FFF\u3400-\u4DBF\u20000-\u2A6DF\u3040-\u309F\u30A0-\u30FF]+$').hasMatch(term)) return true;
    ...
    // よくある除外語
    final excludeTerms = {
      'です', 'ます', 'する', 'なる', 'ある', 'いる', 'ない', 'だ', 'である',
      'から', 'まで', 'より', 'ので', 'のに', 'ため', 'とき', 'こと', 'もの',
      'これ', 'それ', 'あれ', 'どれ', 'ここ', 'そこ', 'あそこ', 'どこ',
      'わたし', 'あなた', 'かれ', 'かのじょ', 'ぼく', 'おれ',
    };
  ...
  static String _extractSnippet(String text, int start, int end) {
    ...
    if (termStart >= 0 && termEnd <= snippet.length) {
      snippet = '${snippet.substring(0, termStart)}【${snippet.substring(termStart, termEnd)}】${snippet.substring(termEnd)}';
    }
